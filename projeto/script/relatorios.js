...
async function listar() {
    ...
    const result = await fetch(`http://localhost:3000/relatorios/${id_usuario}`);
...
async function gerarGrafico(id_ciclo){
    ...
    const gastosRes = await fetch(`http://localhost:3000/gastos/${id_ciclo}`);
    const receitasRes = await fetch(`http://localhost:3000/receita/${id_ciclo}`);
...
async function atualizarciclo(id){
 ...
 await fetch(`http://localhost:3000/abrir-ciclo/${id}`,{
   method:"PUT"
 })
...
async function removerRelatorio(id_relatorio, id_ciclo){
    ...
    await fetch(`http://localhost:3000/relatorio/${id_relatorio}/${id_ciclo}`, {
        method: "DELETE"
    });
